...
const validatePreConditions = (data: SchedulingData) => {
  if (data.session.status === "Locked") { // Assuming 'Locked' means ready? User story says "If not Locked, prompt to lock".
...
const runSchedulingAlgorithm = (data: SchedulingData) => {
  ...
  const isAvailable = (lecturerId: number, dayId: number): boolean => {
      ...
      const availability = lecturer.lecturerDayAvailability.find(a => a.sessionDayId === dayId);
  ...
  for (const day of sessionDays) {
    ...
    while (continueDay && unscheduledTopics.length > 0) {
        ...
        const candidates = lecturers.filter(l => 
            !assignedForDay.has(l.id) && 
            isAvailable(l.id, day.id)
        );
...
const saveSchedule = async (scheduled: ScheduledCouncil[]) => {
    for (const item of scheduled) {
        ...
        const matchesData = item.topics.map((topic) => {
